...
"""
Source: https://github.com/nipy/nipype/blob/master/examples/fmri_ants_openfmri.py (retrieved 5/12/2017)

This script was modified in order to regress out the nuisance predictors (i.e. motion and artifact)
from the preprocessed bold images. The resulting residual maps are intended as input for an
effective connectivity analysis with py-causal.
"""
...
# import our custom workflows (scripts should be in same dir as this one)
from run_flow import create_run_flow
from modelfit_nuisance import create_nuisance_modelfit_workflow
...
def create_reg_workflow(name='registration'):
    ...
    register.connect(inputnode, 'target_image_brain', warpmask, 'reference_image')
    register.connect(mean2anat_mask, 'mask_file', warpmask, 'input_image')
    # apply only the ANTS transformation since the mask is already in subject's space after BBR
    ...
    register.connect(inputnode, 'target_image_brain', warpsegment, 'reference_image')
    ...
    register.connect(stripper, 'out_file', outputnode, 'brain')
    register.connect(warpmask, 'output_image', outputnode, 'mean2anat_mask_mni')
    register.connect(fast, 'partial_volume_files', outputnode, 'anat_segmented')
    register.connect(warpsegment, 'output_image', outputnode, 'anat_segmented_mni')
...
def create_apply_transforms_workflow(name='bold2mni'):
    ...
    register.connect(warpbold, 'output_image', outputnode, 'transformed_files')
...
def analyze_openfmri_dataset(data_dir, subject=None, model_id=None,
    ...
    work_dir : str
        ...
        nuisanceonly : bool
        indicate if given conditions should be ignored in 1st lvl analysis.
        useful if only nuisance regressors should be used.
    """
    ...
    # load custom wf if nuisance only model is wished. else get wf from builtin
    if nuisanceonly:
        modelfit = create_nuisance_modelfit_workflow()
    else:
        modelfit = create_modelfit_workflow()

    fixed_fx = create_run_flow()
    if subjects_dir:
    ...
    def get_contrasts(contrast_file, task_id, conds):
        ...
        print("Got %d contrasts" % (len(contrasts)))
        # add auto contrasts for each column
        for i, cond in enumerate(conds):
            con = [cond, 'T', ['cond%03d' % (i + 1)], [1]]
            contrasts.append(con)
        print("Got %d contrasts after adding %d conds" % (len(contrasts), len(conds)))
        return contrasts
    ...
    # feed residuals to reslice_bold as if they were bold files
    reslice_bold = create_apply_transforms_workflow()
    if nuisanceonly:
        wf.connect(modelfit.get_node('outputspec'), 'residual4d', reslice_bold, 'inputspec.source_files')
    else:
        wf.connect(preproc, 'outputspec.highpassed_files', reslice_bold, 'inputspec.source_files')

    wf.connect(registration, 'outputspec.func2target_transforms', reslice_bold, 'inputspec.transforms')
    ...
    # connect session_info to modelfit
    if nuisanceonly: # through remove conds
        wf.connect(modelspec, 'session_info', removeconds, 'session_info')
        wf.connect(removeconds, 'nuisance_info', modelfit, 'inputspec.session_info')
        modelfit.inputs.inputspec.contrasts = []
    else: # directly
    ...
    def merge_files_nuisance(residual4d, sigmasquareds):
        out_files = []
        splits = []
        out_files.extend(residual4d)
        splits.append(len(residual4d))
        out_files.extend(sigmasquareds)
        splits.append(len(sigmasquareds))
        return out_files, splits
    ...
    def split_files_nuisance(in_files, splits):
        residual4d = in_files[:splits[0]]
        sigmasquareds = in_files[splits[0]:(splits[0]+splits[1])]
        return residual4d, sigmasquareds

    if nuisanceonly:
        mergefunc = pe.Node(niu.Function(input_names=['residual4d', 'sigmasquareds'],
                                         output_names=['out_files', 'splits'],
                                         function=merge_files_nuisance),
                            name='merge_files')
        splitfunc = pe.Node(niu.Function(input_names=['in_files', 'splits'],
                                         output_names=['residual4d', 'sigmasquareds'],
                                         function=split_files_nuisance),
                            name='split_files')
        ...
        # send images through registration using mergefunc before and splitfunc after
    ...
    """
    Check alignment of bet to anatomical
    """

    slicer = pe.Node(fsl.Slicer(image_width=1300, sample_axial=12,
    ...
    """
    Check alignment of bold to anatomical
    """

    slicer_bold = pe.Node(fsl.Slicer(image_width=1300, sample_axial=8,
                                     nearest_neighbour=True, threshold_edges=-150),
                          name='slicer_bold')
    ...
    wf.connect(preproc, 'outputspec.motion_parameters', createfilter1, 'motion_params')
    ...
    # use the resliced bold
    wf.connect(reslice_bold, 'outputspec.transformed_files', filter1, 'in_file')
    wf.connect(reslice_bold, ('outputspec.transformed_files', rename, '_filtermotart'),
    ...
    # pick only white matter of the projected segmentation in mni
    wf.connect(registration, ('outputspec.anat_segmented_mni', selectindex, [2]),
    ...
    def get_subs(subject_id, conds, run_id, model_id, task_id):
        ...
        for i, run_num in enumerate(run_id):
            ...
            subs.append(('_tsnr%d/' % i, '/run%02d_' % run_num))
            ...
            subs.append(('_warpbold%d/bold_dtype_mcf_mask_smooth_mask_gms_tempfilt_maths_trans.nii.gz' % i,
                         'run%03d/bold_mni.nii.gz' % run_num))

            # substitutions for residuals and sigmasquareds
            subs.append(('_modelestimate%d/res4d.nii.gz' % i, 'res4d_run%02d.nii.gz' % run_num))
            subs.append(('_warpbold%d/res4d_trans.nii.gz' % i, 'res4d_run%02d.nii.gz' % run_num))
            subs.append(('_modelestimate%d/sigmasquareds.nii.gz' % i, 'sigmasquareds_run%02d.nii.gz' % run_num))

            subs.append(('_makecompcorrfilter%d/' % i, '/run%02d_' % run_num))
        ...
        subs.append(('/%s' % subject_id, '/model%03d/task%03d/%s' % (model_id, task_id, subject_id)))
    ...
    if not nuisanceonly:
        wf.connect([(fixed_fx.get_node('outputspec'), datasink,
                     # these are from the 2nd lvl model
                     [('res4d', 'res4d'),
                      ('copes', 'copes'),
                      ('varcopes', 'varcopes'),
                      ('zstats', 'zstats'),
    ...
    # sink residuals from 1st lvl nuisance only model
    if nuisanceonly:
        wf.connect(modelfit.get_node('outputspec'), 'residual4d', datasink, 'residual4d')
        wf.connect(modelfit.get_node('outputspec'), 'sigmasquareds', datasink, 'sigmasquareds')
    ...
    wf.connect(registration, 'outputspec.mean2anat_mask_mni', datasink, 'qa.mask.mean2anat_mni')
    ...
    wf.connect(registration, 'outputspec.func2target_transforms', datasink, 'xfm.mean2target')
    wf.connect(registration, 'outputspec.anat2target_transform', datasink, 'xfm.anat2target')

    # connect bold or residuals to datasink, depending on nuisance only specification
    if nuisanceonly:
        wf.connect(reslice_bold, 'outputspec.transformed_files', datasink, 'residual4d.mni')
    else:
        wf.connect(reslice_bold, 'outputspec.transformed_files', datasink, 'bold')

    wf.connect(registration, 'outputspec.anat_segmented', datasink, 'qa.fast')
    wf.connect(registration, 'outputspec.anat_segmented_mni', datasink, 'qa.fast.mni')

    wf.connect(createfilter2, 'out_files', datasink, 'qa.noisecomp')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nuisance_only", default="False",
                        help="Do not run, just write the graph to specified file")
    ...
    nuisance_only_string = args.nuisance_only
    if nuisance_only_string == 'False':
        nuisance = False
    elif nuisance_only_string == 'True':
        nuisance = True
    else:
        raise ValueError('--nuisance_only must be either "True" or "False"')
